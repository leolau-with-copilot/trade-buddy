...
class TestDeferredReflection:
    ...
    def test_resolve_marks_entry_completed(self, tmp_path):
        ...
        mock_graph._fetch_returns = MagicMock(return_value=(0.05, 0.02, 5))
        mock_graph._generate_reflection = MagicMock(return_value="Momentum confirmed.")
        TradingAgentsGraph._resolve_pending_entries(mock_graph, "NVDA")
...
# ---------------------------------------------------------------------------
# get_past_context: ordering, limits, full cycle (used to inject lessons into
# the Judge prompt in the AutoGen pipeline)
# ---------------------------------------------------------------------------
...
class TestLegacyRemoval:
    ...
    def test_full_pipeline_no_regression(self, tmp_path, monkeypatch):
        """propagate() completes and stores the decision after the AutoGen redesign."""
        import functools

        from tradingagents.datastore import AnalysisStore
        from tradingagents.graph.orchestrator import AnalysisResult
        import tradingagents.graph.trading_graph as tg

        result = AnalysisResult(
            company_of_interest="NVDA", trade_date="2026-01-10", asset_type="stock",
            final_trade_decision="Rating: Buy\nBuy NVDA.", rating="Buy",
        )

        async def fake_run_analysis(ctx, analysts, on_event=None):
            return result

        monkeypatch.setattr(tg, "run_analysis", fake_run_analysis)

        mock_graph = MagicMock()
        mock_graph.ticker = None
        mock_graph.memory_log = TradingMemoryLog({"memory_log_path": str(tmp_path / "mem.md")})
        mock_graph.store = AnalysisStore(str(tmp_path / "store.db"))
        mock_graph.log_states_dict = {}
        mock_graph.debug = False
        mock_graph.selected_analysts = ["market"]
        mock_graph.config = {"results_dir": str(tmp_path)}
        mock_graph.chat_client = MagicMock()
        mock_graph.deep_client = MagicMock()
        mock_graph.process_signal.return_value = "Buy"
        # Bind the real helpers propagate() calls so the write path runs for real.
        for name in ("_resolve_pending_entries", "_resolve_pending_store_outcomes", "_log_state"):
            setattr(mock_graph, name, functools.partial(getattr(TradingAgentsGraph, name), mock_graph))

        final_state, decision = TradingAgentsGraph.propagate(mock_graph, "NVDA", "2026-01-10")
        assert decision == "Buy"
