...
## [Unreleased] — AutoGen rearchitecture

### Added (later)
- **Web UI** (`webapp/`): a FastAPI backend + single-page frontend (`python -m webapp`
  or `tradingagents-web`). Top: a searchable price chart (case-insensitive
  name→symbol autocomplete via Yahoo; 5D/1W/1M/3M/1Y/5Y/MAX ranges). Left: an
  animated agent-coordination graph — a glowing "electron" carries a summarized
  topic from each agent to the next as stages complete. Right: a live output
  window streaming each agent's full markdown, plus the Judge's scoreboard and a
  −1…+1 weighted-score bar. Driven by a Server-Sent-Events `/api/analyze` stream
  off the same `on_event` callback the CLI uses.
- **OpenRouter and Ollama providers** alongside DeepSeek (all OpenAI-compatible,
  one client). `create_model_client(model, provider=...)`.
- **Self-examining Tree-of-Thoughts**: researchers generate more candidate lines
  (`tot_breadth` default 4), each self-assesses a *likelihood* it holds plus a
  *critique* of its own weakness, and the synthesis is weighted by those
  likelihoods and must address the weaknesses.

### Changed (later)
- **Trimmed to three providers** (DeepSeek / OpenRouter / Ollama). The entire
  LangChain client layer (`tradingagents/llm_clients/`) and the
  `langchain-*` / `langgraph` dependencies are **removed**; the CLI model picker
  is rewritten accordingly. (Previously these were retained for the picker.)
- **CLI timer no longer freezes**: the pipeline runs on a worker thread while the
  TUI repaints at ~5 fps, so the elapsed clock ticks and status/reports stream in
  continuously instead of only updating between stages.
- Pruned provider-specific tests and stale files (`build/`, `test.py`).

### Changed (breaking)
- **Agent communication moved from LangChain/LangGraph to AutoGen 0.4**
  (`autogen-agentchat`). The old `StateGraph` of node functions that mutated a
  shared `AgentState` is replaced by AutoGen `AssistantAgent`s and a hand-written
  async orchestrator (`tradingagents/graph/orchestrator.py`). The public API is
  unchanged: `TradingAgentsGraph(...).propagate(ticker, date)` still returns
  `(final_state, decision)` (a sync wrapper drives the async pipeline).
- **DeepSeek only.** Models run over DeepSeek's OpenAI-compatible endpoint via a
  single `OpenAIChatCompletionClient` (`tradingagents/autogen_clients/`). Defaults
  are now `deepseek-reasoner` (deep/reasoning) and `deepseek-chat` (quick/tools).
- **New decision topology.** The risk-management team (aggressive/conservative/
  neutral), the Portfolio Manager, the Trader, and the Research Manager are
  **removed**. The new flow is: analysts → bull/bear **Tree-of-Thoughts** debate →
  a single **Judge** that is the sole verdict.

### Added
- **Tree-of-Thoughts researchers** (`autogen_agents/tot.py`, `researchers.py`):
  bull and bear each run a ToT search (configurable `tot_breadth`/`tot_depth`/
  `tot_keep`) and cite technical signals.
- **Technical win-rate backtest** (`tradingagents/backtest/winrate.py`): the
  researchers' "technical intuition" is grounded in the real historical hit rate
  of named signals (e.g. `macd_bull_cross`, `golden_cross`); hallucinated signal
  names are dropped. Tunable via `winrate_horizon_days` / `winrate_lookback_years`.
- **Judge + scoreboard** (`autogen_agents/judge.py`): verifies the researchers'
  claims against the technical/fundamental reports, checks feasibility via a
  DuckDuckGo `web_search` tool and the historical dataset, builds a weighted
  scoreboard, and derives the rating from the weighted score. Falls back to
  free-text + rating heuristic if structured output is unavailable.
- **SQLite analysis store** (`tradingagents/datastore/`): a growing dataset of
  analyses, scoreboards, cited signals, realised outcomes, and a win-rate cache.
  Outcomes are resolved on the next same-ticker run (mirroring the markdown log).
- New deps: `autogen-agentchat`, `autogen-ext[openai]`, `ddgs`. Tests add
  `pytest-asyncio` (`asyncio_mode = "auto"`).

### Fixed
- **DeepSeek has no `json_schema` structured-output mode.** Passing
  `json_output=Schema` / `output_content_type` made AutoGen send an OpenAI
  `json_schema` `response_format`, which DeepSeek rejects ("This response_format
  type is unavailable now"); the reasoner supports no JSON mode at all. Typed
  output is now obtained by embedding the schema in the prompt and parsing the
  returned JSON (`autogen_agents/structured.py::complete_json`), used by the ToT
  researchers and a JSON-formalisation pass after the Judge's tool-using
  verification. `ModelInfo.structured_output` is set to `False`.
- **Live CLI report panel was empty during a run.** The orchestrator now streams
  each finished section's markdown (analyst report / bull & bear case / verdict)
  through the `on_event(stage, status, content)` callback, so the "Current Report"
  panel fills in as stages complete instead of only at the end.
- **DeepSeek thinking-mode `reasoning_content` round-trip.** DeepSeek's thinking
  models require `reasoning_content` to be echoed back on the assistant message in
  every subsequent turn; AutoGen captures it into `AssistantMessage.thought` but
  does not re-emit it, so the analysts'/Judge's multi-turn tool loops failed with
  HTTP 400 ("The `reasoning_content` in the thinking mode must be passed back to
  the API."). `_DeepSeekChatCompletionClient` re-attaches it at the
  `_process_create_args` seam (no-op for non-thinking turns).

### Removed
- LangGraph checkpoint-resume (`--checkpoint` / `--clear-checkpoints`) — it was
  LangGraph-specific. The markdown `TradingMemoryLog` is retained; the new SQLite
  store is the structured dataset.

### Notes
- The LangChain client layer (`tradingagents/llm_clients/`) and the
  `langchain-*` / `langgraph` dependencies are retained only to power the CLI's
  interactive provider/model picker and a startup warning filter; they are **not
  used at inference time**. A later pass can remove them once the picker is
  reworked for DeepSeek-only.
